"""
This script is for an IoT system that generates transactions, sends them to master nodes, and manages the state of the network. It includes functionalities for sending data to a server, handling MQTT messages, maintaining a DAG, and visualizing node reputations in real-time.
"""
...
# Set up the logger
logging.basicConfig(filename='master_node_events.log', level=logging.INFO, format='%(asctime)s - %(message)s')

def log_event(event, details):
    logging.info(f"{event} - {details}")

# Configuration and Initialization
node_id = "Master_1"  # Change node ID as needed
topic_temperature = "temperature1"  # Change topic as needed
...
def send_data_to_server(transaction_data, signature, public_key, node_id):
    """
    Sends transaction data to a server and returns the response.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Error sending data to server: status code {response.status_code}, response '{response.text}'")
            return None
...
def periodic_visualization(graph):
    ...
    # Uncomment for DAG visualization
...
def handle_presence_message(data):
    ...
    if data['action'] == "join" and node != node_id:
        if node not in other_nodes:
            ...
            log_event('Node Joined', f"Node ID: {node}")
        ...
        if node in other_nodes:
            ...
            log_event('Node Left', f"Node ID: {node}")
...
def add_transaction_to_dag(transaction_data):
    ...
    # Send data to server including signature and public key
    received_hash = send_data_to_server(transaction_data['data'], signature, public_key_pem, node_id)
    ...
    hash_input = f"{transaction_data['data']['transaction_id']}{transaction_data['data']['device_id']}{transaction_data['data']['temperature']}{transaction_data['data']['timestamp']}".encode('utf-8')
    local_hash = hashlib.sha256(hash_input).hexdigest()
    ...
    if local_hash == received_hash:
        ...
        print("Verification error: local hash does not match received hash.")
    return None
...
def process_peer_message(data):
    ...
    # Calculate local hash to verify message integrity
    hash_input = f"{data['transaction_id']}{data['device_id']}{data['temperature']}{data['timestamp']}".encode('utf-8')
    local_hash = hashlib.sha256(hash_input).hexdigest()
    ...
    if local_hash == received_hash:
        ...
        print("Error: Received hash does not match locally calculated hash.")
...
def update_reputation(node_id, change):
    ...
    for node, rep in reputation.items():
        log_event('Current Reputation', f"Node: {node}, Reputation: {rep}")
...
def select_authority():
    ...
    log_event('Authority Change', f"New Authority: {selected_node}")
