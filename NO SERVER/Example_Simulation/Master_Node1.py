"""
This script sets up a master node for an IoT system that generates transactions, sends them to other master nodes, and manages the state of the network. It includes functionalities for handling MQTT messages, maintaining a DAG, and visualizing node reputations in real-time.
"""
...
def visualize_dag(nx_graph):
    ...
    labels = {node: node for node in nx_graph.nodes()}  # Use the full label of each node
...
def process_iot_message(data):
    ...
    if validate_transaction(data['data'], data['signature'], data['public_key']):
        ...
        if authority == node_id:
            ...
            add_transaction_to_dag(data)  # Assumes it returns the new node hash
        else:
...
def add_transaction_to_dag(transaction_data):
    ...
    if transaction_id not in dag.nodes:
        dag.add_node(transaction_id, transaction_data)
        transactions_added_to_dag += 1
        print(f"Transaction {transaction_id} added to the DAG")
        notify_peers(transaction_data)
        dag.assign_parents(transaction_id)  # Ensure you are passing the ID correctly
...
def process_peer_message(data):
    ...
    if transaction_id not in dag.nodes:
        dag.add_node(transaction_id, data)  # Add the node to the DAG
        dag.assign_parents(transaction_id)  # Assign parents after adding the node
        transactions_added_to_dag += 1
...
def update_reputation(node_id, change):
    ...
    #log_event('Reputation Update', f"Node: {node_id}, New Reputation: {reputation[node_id]}")
...
def notify_peers(transaction_data):
    """
    Notifies other peers about a validated transaction.
    """
    client.publish("network/validated_transactions", json.dumps(transaction_data))
...
def animate(i):
    ...
    filename = os.path.join(current_dir, f"reputation_chart_{time.time()}.png")
